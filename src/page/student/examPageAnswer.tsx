...
import { getSoalExamStudent } from '../../models/Exam';
import {
  createExamStudentAnswer,
  iCreate_Exam_student_answer,
} from '../../models/examAnswer';
import useStore from '../../store/globalState';

const ExamPageAnswer = () => {
  ...
  async function soalDiangostik() {
    const data = await getSoalExamStudent(id_student);
  ...
  const studentAnswer = React.useRef<iCreate_Exam_student_answer[]>([]);
  ...
  const addNewStudentAnswer = (id_question_TryOut: number, answer: string) => {
    if (studentAnswer.current.length !== 0) {
      const data = studentAnswer.current.filter((e) => {
        return e.id_Exam_Question !== id_question_TryOut;
      });
      data.push({
        id_Exam_Question: id_question_TryOut,
  ...
  const submitAnswer = async () => {
    if (TryOutQuestion?.length !== studentAnswer.current.length) {
      ...
      const data = await createExamStudentAnswer(newAnswer, token);
      if (data) {
        setLoading(false);
        refreshPage();
      }
        ...
        {studentStatus === 0 ? (
          <>
            <Typography align='center' variant='h2' sx={{ mt: 5 }}>
              Kamu belum mengerjakan soal Ujian!
            </Typography>
            <Button
              sx={{
                mt: 3,
